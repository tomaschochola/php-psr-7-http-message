...
This package delivers a lightweight, strictly typed implementation of the PSR-7 HTTP Message interface. It provides the immutable value objects representing HTTP requests, responses, URIs, and streams, serving as the foundational data structure for the entire web stack.

Performance is achieved through a minimalist design that avoids unnecessary property hooks or validation logic where strictly typed signatures suffice. The implementation ensures total immutability, guaranteeing thread-safety and predictable state management across the entire HTTP request lifecycle.
